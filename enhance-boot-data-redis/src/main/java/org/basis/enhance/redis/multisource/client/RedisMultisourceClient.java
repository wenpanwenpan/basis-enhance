...
public class RedisMultisourceClient {
    ...
    public RedisTemplate<String, String> opsDefaultDb(String datasource) {
        ...
        RedisTemplate<String, String> redisTemplate = RedisDataSourceRegister.getRedisTemplate(datasource + EnhanceRedisConstants.MultiSource.REDIS_TEMPLATE);
    ...
    private AbstractOptionsRedisDb<String, String> commonOpsDb(String datasource) {
        ...
        RedisHelper redisHelper = RedisDataSourceRegister.getRedisHelper(datasource + EnhanceRedisConstants.MultiSource.REDIS_HELPER);
        if (redisHelper == null) {
            throw new RuntimeException("没有该数据源，请确认传入的redis数据源名称是否正确.");
        }
